...
int c = 1;
int L[N*20],R[N*20];
vector<pair<int,int>>tree(N*20,NIL),lazy(N*20,NIL);
struct segtree{
    ...
    void propagate(int x, int lx, int rx)
    {
        if(lazy[x] != NIL)
        {
            ...
            if(lx != rx)
            {
                lazy[create(L[x])] = max(lazy[create(L[x])],lazy[x]);
                lazy[create(R[x])] = max(lazy[create(R[x])],lazy[x]);
            }
            lazy[x] = NIL;
        }
    }
    ...
    void update(int x, int lx, int rx, int l, int r, pair<int,int> v)
    {
        ...
        tree[x] = merge(tree[create(L[x])], tree[create(R[x])]);
    }
    ...
    pair<int,int> query(int l, int r, int x, int lx, int rx)
    {
        ...
        int mid = (lx+rx)/2;
        pair<int,int> s1 = query(l,r,L[x],lx,mid);
        pair<int,int> s2 = query(l,r,R[x],mid+1,rx);
        return merge(s1,s2);
    }
